module pyth::price_info {
    ...
    use haneul::dynamic_object_field::{Self};
    use haneul::table::{Self};
    ...
    /// Haneul object version of PriceInfo.
    ...
    /// UID (in bytes) of the corresponding Haneul PriceInfoObject.
    ...
    public fun get_balance(price_info_object: &PriceInfoObject): u64 {
        if (!dynamic_object_field::exists_with_type<vector<u8>, Coin<HANEUL>>(&price_info_object.id, FEE_STORAGE_KEY)) {
            return 0
        };
        let fee = dynamic_object_field::borrow<vector<u8>, Coin<HANEUL>>(&price_info_object.id, FEE_STORAGE_KEY);
        coin::value(fee)
    }

    public fun deposit_fee_coins(price_info_object: &mut PriceInfoObject, fee_coins: Coin<HANEUL>) {
        if (!dynamic_object_field::exists_with_type<vector<u8>, Coin<HANEUL>>(&price_info_object.id, FEE_STORAGE_KEY)) {
            dynamic_object_field::add(&mut price_info_object.id, FEE_STORAGE_KEY, fee_coins);
        }
        else {
            let current_fee = dynamic_object_field::borrow_mut<vector<u8>, Coin<HANEUL>>(
