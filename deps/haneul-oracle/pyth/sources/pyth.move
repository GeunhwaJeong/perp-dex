module pyth::pyth {
    ...
    fun create_and_share_price_feeds_using_verified_price_infos(latest_only: &LatestOnly, pyth_state: &mut PythState, price_infos: vector<PriceInfo>, ctx: &mut TxContext){
        while (!vector::is_empty(&price_infos)){
            ...
            // Only create new Haneul PriceInfoObject if not already
            ...
            if (!state::price_feed_object_exists(
                ...
                // Create and share newly created Haneul PriceInfoObject containing a price feed,
    ...
    public fun update_single_price_feed(
        ...
        // On Haneul, users get to choose which price feeds to update. They specify a single price feed to
    ...
    ///
    /// The price_info_object is a Haneul object with the key ability that uniquely
...
module pyth::pyth_tests{
    ...
    use haneul::haneul::HANEUL;
    use haneul::coin::{Self, Coin};
    use haneul::test_scenario::{Self, Scenario, ctx, take_shared, return_shared};
    ...
    /// Init Pyth state.
    /// Set initial Haneul clock time.
    /// Mint some HANEUL fee coins.
    public fun setup_test(
        ...
        let coins = coin::mint_for_testing<HANEUL>(to_mint, ctx(&mut scenario));
    ...
    fun test_get_update_fee() {
        ...
        coin::burn_for_testing<HANEUL>(test_coins);
    ...
    fun test_create_and_update_price_feeds_with_batch_attestation_success() {
        ...
        // Create vector of price info objects (Haneul objects with key ability and living in global store),
    ...
    fun test_create_and_update_single_price_feed_with_accumulator_failure() {
        ...
        // clean up test scenario
        test_scenario::next_tx(&mut scenario, DEPLOYER);
        coin::burn_for_testing<HANEUL>(coins);
    ...
    fun test_parse_and_verify_accumulator_updates(){
        use haneul::test_scenario::{Self, take_shared, return_shared};
        use haneul::transfer::{Self};
