...
module wormhole::publish_message {
    ...
    /// This type is emitted via `haneul::event` module. Guardians pick up this
    ...
    /// `publish_message` emits a message as a Haneul event. This method uses the
    ...
    public fun publish_message(
        ...
        // Haneul is an instant finality chain, so we don't need confirmations.
        let consistency_level = 0;

        // Emit Haneul event with `WormholeMessage`.
        haneul::event::emit(
            WormholeMessage {
                sender,
                sequence,
...
module wormhole::publish_message_tests {
    ...
    fun test_publish_message() {
...

        {
            ...
            haneul::transfer::public_transfer(emitter_cap, user);
        };
