...
/// This module implements a container that collects fees in HANEUL denomination.
...
module wormhole::fee_collector {
    ...
    /// Container for configured `fee_amount` and `balance` of HANEUL collected.
    ...
    /// Retrieve current HANEUL balance.
    public fun balance_value(self: &FeeCollector): u64 {
        balance::value(&self.balance)
    }

    /// Take `Balance<HANEUL>` and add it to current collected balance.
    public fun deposit_balance(self: &mut FeeCollector, fee: Balance<HANEUL>) {
        assert!(balance::value(&fee) == self.fee_amount, E_INCORRECT_FEE);
        balance::join(&mut self.balance, fee);
    }

    /// Take `Coin<HANEUL>` and add it to current collected balance.
    public fun deposit(self: &mut FeeCollector, fee: Coin<HANEUL>) {
        deposit_balance(self, coin::into_balance(fee))
    }

    /// Create `Balance<HANEUL>` of some `amount` by taking from collected balance.
    public fun withdraw_balance(
        self: &mut FeeCollector,
        amount: u64
    ): Balance<HANEUL> {
        // This will trigger `haneul::balance::ENotEnough` if amount > balance.
        balance::split(&mut self.balance, amount)
    }

    /// Create `Coin<HANEUL>` of some `amount` by taking from collected balance.
    public fun withdraw(
        self: &mut FeeCollector,
        amount: u64,
        ctx: &mut TxContext
