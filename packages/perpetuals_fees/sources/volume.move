...
/// A rolling window of per-epoch taker volume. Bucket `epoch % length` holds the volume of the
/// epoch recorded in `bucket_epochs`; a bucket whose epoch has fallen out of the window is
/// ignored on read and overwritten on the next write, so no rotation step is needed.
